...
describe('parseIncompleteMarkdown', () => {
	describe('basic input handling', () => {
		it('should return non-string inputs unchanged', () => {
			expect(parseIncompleteMarkdown(null as unknown as string)).toBe(null);
			expect(parseIncompleteMarkdown(undefined as unknown as string)).toBe(undefined);
			expect(parseIncompleteMarkdown(123 as unknown as string)).toBe(123);
		});
	...
	describe('bold-italic formatting (***)', () => {
		...
		it('should handle triple asterisks at start of text', () => {
			expect(parseIncompleteMarkdown('***Starting bold-italic')).toBe('***Starting bold-italic***');
		});
	...
	describe('KaTeX block formatting ($$)', () => {
		it('should complete incomplete block KaTeX', () => {
			expect(parseIncompleteMarkdown('Text with $$formula')).toBe('Text with $$formula$$');
			expect(parseIncompleteMarkdown('$$incomplete')).toBe('$$incomplete$$');
		});
	...
	describe('KaTeX inline formatting ($)', () => {
		...
		it('should not complete single $ but should complete block $$', () => {
			...
			expect(parseIncompleteMarkdown('$$block$$ and $inline')).toBe('$$block$$ and $inline');
		});
	...
	describe('chunked streaming scenarios', () => {
		it('should handle partial bold text at chunk boundary', () => {
			expect(parseIncompleteMarkdown('Here is some **bold tex')).toBe('Here is some **bold tex**');
		});
		...
		it('should handle blockquotes with incomplete formatting', () => {
			expect(parseIncompleteMarkdown('> Quote with **bold')).toBe('> Quote with **bold**');
		});
		...
		it('should handle complex nested structures from chunks', () => {
			...
			expect(parseIncompleteMarkdown('1. First item\n   - Nested with `code\n2. Second')).toBe(
				'1. First item\n   - Nested with `code\n2. Second`'
			);
		});
		...
		it('should handle multiple incomplete formats in one chunk', () => {
			...
			expect(parseIncompleteMarkdown('Text **bold `code')).toBe('Text **bold `code**`');
		});
	});

	describe('mixed formatting scenarios', () => {
		it('should handle bold inside italic', () => {
			expect(parseIncompleteMarkdown('*italic with **bold')).toBe('*italic with **bold***');
		});

		it('should handle code inside bold', () => {
			// Bold gets closed first, then code
			expect(parseIncompleteMarkdown('**bold with `code')).toBe('**bold with `code**`');
		});

		it('should handle strikethrough with other formatting', () => {
			// Both formats get closed
			expect(parseIncompleteMarkdown('~~strike with **bold')).toBe('~~strike with **bold**~~');
		});

		it('should handle dollar sign inside other formatting', () => {
			// Bold gets closed, dollar sign stays as-is (likely currency)
			expect(parseIncompleteMarkdown('**bold with $x^2')).toBe('**bold with $x^2**');
		});
	...
	describe('real-world streaming chunks', () => {
		it('should handle typical GPT response chunks', () => {
			...
			expect(parseIncompleteMarkdown(chunks[2])).toBe('Here is a **bold statement**');
			expect(parseIncompleteMarkdown(chunks[3])).toBe('Here is a **bold statement** about');
			expect(parseIncompleteMarkdown(chunks[4])).toBe('Here is a **bold statement** about `code`');
		...
		it('should handle code explanation chunks', () => {
			...
			expect(parseIncompleteMarkdown(chunks[1])).toBe('To use this function, call `getData(`');
		...
		it('should handle bold-italic chunks', () => {
			...
			expect(parseIncompleteMarkdown(chunks[2])).toBe('This is ***very important***');
