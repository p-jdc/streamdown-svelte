...
describe('Table components', () => {
	...
	beforeEach(() => {
		// Mock global ClipboardItem constructor
		// Assign a mock ClipboardItem on globalThis using unknown/Record to avoid `any`
	...
	describe('Copy functionality', () => {
		it('should copy table as markdown by default', async () => {
			...
			// Verify clipboard was called with ClipboardItem containing markdown format
			const expectedMarkdown = '| Name | Age |\n| --- | --- |\n| John | 25 |\n| Jane | 30 |';
			expect(getClipboardMock().write).toHaveBeenCalledWith([
				expect.objectContaining({
					data: expect.objectContaining({
						'text/plain': expectedMarkdown,
		...
		it('should copy table as CSV format', async () => {
			...
			// Verify clipboard was called with ClipboardItem containing CSV format
			const expectedCSV = 'Name,Age\nJohn,25';
			expect(getClipboardMock().write).toHaveBeenCalledWith([
				expect.objectContaining({
					data: expect.objectContaining({
						'text/plain': expectedCSV,
						'text/html': expect.any(Blob)
					})
				})
			]);
		...
		it('should handle clipboard API not available', async () => {
			...
			// Mock clipboard API as not available completely
			Object.defineProperty(navigator, 'clipboard', {
				value: {
					write: undefined
				},
		...
		it('should handle CSV special characters correctly', async () => {
			...
			// Should properly escape CSV special characters - only strings with commas, quotes, or newlines are escaped
