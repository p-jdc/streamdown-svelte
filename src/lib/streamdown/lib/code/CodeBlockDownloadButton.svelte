...
<script lang="ts">
	...
	import { CODE_BLOCK_CONTEXT_KEY, type CodeBlockContextType } from './types';
	...
	if (!context && !propCode) {
		throw new Error(
			'CodeBlockDownloadButton must be used within a CodeBlock or have code prop provided'
		);
	}

	// Use prop code if provided, otherwise fall back to context
	const codeToUse = $derived(propCode ?? context?.code ?? '');
